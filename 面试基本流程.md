...
String类中使用字符数组保存字符串，private　final　char　value[]，所以 string对象是不可变的。StringBuilder与StringBuffer都继承自AbstractStringBuilder类，在AbstractStringBuilder中也是使用字符数组保存字符串，char[] value，这两种对象都是可变的。线程安全性
...
![image-20220315165122978](./img/image-20220315165122978.png)
...
[http tcp，协议4和7层架构](https://blog.csdn.net/h2604396739/article/details/85223983)
...
主从集群/主主集群，保证高可用；避免读写性能；分库分表

3、MySQL程序配置优化

my.cnf(mysql5.7默认最大连接数157)；binlog日志，默认不开启；缓存池默认大小

4、sql执行优化

慢sql定位排查，慢日志；explain结果分析；show profile 分析会话sql语句情况
...
- 树的结点树可以为0，对于这种树，我们称为**空树**
- 树与图的区别在于**树中没有一个闭环**

二叉树：即每个结点都最多只有两个子结点的树。缺点：**有序数据会变成链表**。

平衡二叉树：任一结点的左右子树的高度差绝对值不超过1，且左右子树均为平衡二叉树（*防止树退化成链表*）为了减少二叉查找树层次，提高查找速度。平衡二叉树追求绝对平衡，条件比较苛刻，实现起来比较麻烦，**每次插入新节点之后需要旋转的次数不能预知**。
...
最左前缀匹配原则，非常重要的原则，mysql会一直向右匹配直到遇到范围查询(>、<、between、like)就停止匹配，比如a=1 and b=2 and c>3 andd=4如果建立(a,b,c,d)顺序的索引，d是用不到索引的，如果建立(a,b,d,c)的索引则都可以用到，a,b,d的顺序可以任意调整。
...
所谓的next key lock就是一个行锁（record lock）+范围锁（gap lock），比如某一个辅助索引（比如上面的class_id），如果它有1,3,5这几个值，那么当我们使用next key lock的锁住class_id=1的时候，实际上锁住了（-无穷，1]，或者锁住class_id=3的时候，实际上锁住的是（1,3]，也就是一个左开右闭的区间。如果此时别的事务要在这个区间内插入数据，就会被阻塞住。这个锁一直到事务提交才会释放。因此，即使出现了上面图片里面这种情况，也可以保证前后两次去读的内容一致，因为对这个辅助索引上的锁是：“next key lock”，他会锁住一个区间。
 ...
 但是注意，对于可重复读默认使用的就是next key lock(InnoDB默认开启)，但是对于“唯一索引”，比如主键的索引，next key lock会降级成行锁，而不会锁住一个区间。因此，如果上面的事务1的update使用的是主键，事务2也使用主键进行插入，那么实际上事务2根本不会被阻塞，可以立即插入并返回。而对于非唯一索引，next key lock则不会降级。

间隙锁`InnoDB`存储引擎在 RR 级别下通过 `MVCC`和 `Next-key Lock` 来解决幻读问题：
...
1、集群管理
在多个节点的集群中，为了保证集群的HA特性(高可用)，每个节点都会冗余一份数据副本，就会产生数据同步问题(保证数据为最新)。

2、分布式锁
跨进程的共享资源的并发安全。

3、Master选举
在集群中，为了降低集群数据同步的复杂度，一般会存在 Master 和 Slave，
...
[zookeeper面试专题及答案](zookeeper面试专题及答案.md)

1、ZK本质上不是强一致性模型，而是顺序一致性模型，需要调用`sync`方法进行同步。Zookeeper(CP) 为了保证一致性，在所有节点同步完成之前是阻塞状态的。基于 zookeeper 来开发 HA 高可用机制，就是一个**重要进程一般会做主备**两个，主进程挂了立马通过 zookeeper 感知到切换到备用进程。
...
一个机器接收到了请求之后先获取 zookeeper 上的一把分布式锁，就是可以去创建一个 有序znode，接着执行操作；然后另外一个机器也**尝试去创建**那个 znode，结果发现自己创建不了，因为被别人创建了，那只能等着，等第一个机器执行完了自己再执行。

3、利用持久化节点储存和管理，集群节点信息。
